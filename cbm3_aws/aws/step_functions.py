...
def _create_worker_activity(client: SFNClient, names):
    response = client.create_activity(name=names.run_activity)
    return response["activityArn"]


def _create_task_state_machine(
    client: SFNClient, worker_activity_resource_arn: str, role_arn: str, names
):
...
def _create_application_state_machine(
    client: SFNClient, task_state_machine_arn, role_arn, names
):
...
def cleanup(client: mypy_boto3_stepfunctions.client.SFNClient, arn_context):
